...
// authedReq creates a request with testHandlerActorID injected into the context.
...
// ── T-R38d: GET returns 404 user_role_not_found ───────────────────────────────

func TestHandler_GetUserRole_NotFound(t *testing.T) {
	t.Parallel()
	svc := &rbacsharedtest.UserRolesFakeServicer{
	...
	r := injectUserRoleChi(
		httptest.NewRequest(http.MethodGet, "/rbac/users/"+testHandlerTargetID+"/role", nil),
...
// ── T-R38: PUT returns 409 cannot_reassign_owner ──────────────────────────────
...
func TestHandler_AssignRole_CannotReassignOwner(t *testing.T) {
	...
	r := injectUserRoleChi(
		authedUserRoleReq(t, http.MethodPut, "/rbac/users/"+testHandlerTargetID+"/role", body),
		map[string]string{"user_id": testHandlerTargetID},
	)
	h.AssignRole(w, r)
...
// ── T-R38b: PUT returns 409 cannot_modify_own_role ────────────────────────────
...
// ── PUT returns 422 role_not_found ────────────────────────────────────────────
...
// ── PUT returns 401 when no user ID in context ────────────────────────────────

func TestHandler_AssignRole_NoAuth(t *testing.T) {
	t.Parallel()
	h := userroles.NewHandler(&rbacsharedtest.UserRolesFakeServicer{})
	w := httptest.NewRecorder()
	body := jsonBuf(t, map[string]any{"role_id": testHandlerRoleID, "granted_reason": "test"})
	r := injectUserRoleChi(
		httptest.NewRequest(http.MethodPut, "/rbac/users/"+testHandlerTargetID+"/role", body),
		map[string]string{"user_id": testHandlerTargetID},
	)
	h.AssignRole(w, r)
...
// ── T-R38c: DELETE returns 409 last_owner_removal ────────────────────────────
...
func TestHandler_RemoveRole_LastOwnerRemoval(t *testing.T) {
	...
	w := httptest.NewRecorder()
	r := injectUserRoleChi(
		authedUserRoleReq(t, http.MethodDelete, "/rbac/users/"+testHandlerTargetID+"/role", nil),
		map[string]string{"user_id": testHandlerTargetID},
	)
	h.RemoveRole(w, r)
...
// ── DELETE returns 404 user_role_not_found ────────────────────────────────────
...
// ── FIX 13a: GET returns 500 internal_error ────────────────────────────────────
...
// ── FIX 13b: PUT returns 422 role_id_required when body has empty role_id ───────

func TestHandler_AssignRole_RoleIDRequired(t *testing.T) {
	t.Parallel()
	// No service call expected — validation fires before the service.
	h := userroles.NewHandler(&rbacsharedtest.UserRolesFakeServicer{})
	w := httptest.NewRecorder()
...
// ── FIX 13c: PUT returns 422 granted_reason_required ─────────────────────────
...
// ── FIX 13d: PUT returns 500 internal_error ───────────────────────────────────
...
// ── FIX 13e: PUT returns 4xx on malformed JSON body ───────────────────────────
...
func TestHandler_AssignRole_MalformedBody(t *testing.T) {
	...
	w := httptest.NewRecorder()
	r := injectUserRoleChi(
		authedUserRoleReq(t, http.MethodPut, "/rbac/users/"+testHandlerTargetID+"/role",
...
// ── FIX 13f: DELETE returns 409 cannot_modify_own_role ───────────────────────
...
// ── FIX 13g: DELETE returns 409 cannot_reassign_owner ────────────────────────
...
// ── FIX 13h: DELETE returns 500 internal_error ───────────────────────────────
...
// ── DELETE returns 401 when no user ID in context ─────────────────────────────
...
func TestHandler_RemoveRole_NoAuth(t *testing.T) {
	...
	w := httptest.NewRecorder()
	r := injectUserRoleChi(
		httptest.NewRequest(http.MethodDelete, "/rbac/users/"+testHandlerTargetID+"/role", nil),
		map[string]string{"user_id": testHandlerTargetID},
	)
	h.RemoveRole(w, r)
