...
TEST_SUPPORT_GO_RBAC := internal/db/rbac_test.sql.go

sqlc-generate: ## Generate Go code from SQL queries, then tag all *_test.sql.go files
ifeq ($(DETECTED_OS),Windows)
	...
	@$$content = Get-Content $(TEST_SUPPORT_GO_RBAC) -Raw; if ($$content -notmatch '^//go:build integration_test') { Set-Content $(TEST_SUPPORT_GO_RBAC) ("//go:build integration_test`n`n" + $$content) -NoNewline; Write-Host "[OK] Patched build tag onto $(TEST_SUPPORT_GO_RBAC)" -ForegroundColor Green } else { Write-Host "[OK] Build tag already present" -ForegroundColor Yellow }
	...
	@if ! grep -q '//go:build integration_test' $(TEST_SUPPORT_GO_RBAC); then \
		{ printf '//go:build integration_test\n\n'; cat $(TEST_SUPPORT_GO_RBAC); } > $(TEST_SUPPORT_GO_RBAC).tmp && mv $(TEST_SUPPORT_GO_RBAC).tmp $(TEST_SUPPORT_GO_RBAC); \
		echo "[OK] Patched build tag onto $(TEST_SUPPORT_GO_RBAC)"; \
	else \
		echo "[OK] Build tag already present"; \
	fi
endif
