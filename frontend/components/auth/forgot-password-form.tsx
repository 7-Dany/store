...
import { useVerifyResetCode } from "@/hooks/auth/use-verify-reset-code";
import { useResetPassword } from "@/hooks/auth/use-reset-password";
import { useForgotPasswordResend } from "@/hooks/auth/use-forgot-password-resend";
import {
  IconArrowRight,
  IconArrowLeft,
  IconCheck,
...
export function ForgotPasswordForm({ initialEmail = "" }: ForgotPasswordFormProps) {
  ...
  const { execute: sendCode, isPending: sending, error: sendError, clearError: clearSendError } =
    useForgotPassword({ onSuccess: () => advance() });

  const { execute: verifyCode, isPending: verifying, error: verifyError, clearError: clearVerifyError } =
    useVerifyResetCode({ onSuccess: (token) => { setResetToken(token); advance(); } });

  const { execute: resetPwd, isPending: resetting, error: resetError, clearError: clearResetError } =
    useResetPassword();

  const { resend, isPending: resending, countdown } = useForgotPasswordResend();

  useEffect(() => { if (sendError)   emailForm.setError("email",       { message: sendError });   }, [sendError,   emailForm]);
  useEffect(() => { if (verifyError) otpForm.setError("otp",           { message: verifyError }); }, [verifyError, otpForm]);
  useEffect(() => { if (resetError)  passwordForm.setError("password", { message: resetError });  }, [resetError,  passwordForm]);
  useEffect(() => { if (step.key === "otp") countdown.start(60); }, [stepIndex]); // eslint-disable-line react-hooks/exhaustive-deps

  const isPending = sending || verifying || resetting || resending;

  function advance() { clearErrors(); setDirection("forward"); setStepIndex((i) => i + 1); }
  function back()    { clearErrors(); setDirection("back");    setStepIndex((i) => Math.max(0, i - 1)); }

  function clearErrors() {
    clearSendError(); clearVerifyError(); clearResetError();
    emailForm.clearErrors(); otpForm.clearErrors(); passwordForm.clearErrors();
  }
    ...
    return (
      <form onSubmit={emailForm.handleSubmit((d) => { setEmail(d.email.trim()); sendCode(d.email.trim()); })} noValidate className="flex flex-col gap-5">
        <div className="grid grid-cols-3 items-center">
          <span />
          <StepDots total={STEPS.length} current={0} className="justify-self-center" />
        </div>
              ...
              render={({ field, fieldState }) => (
                <Field data-invalid={fieldState.invalid}>
                  <Input
                    ...
                    disabled={sending}
                    onChange={(e) => { clearErrors(); field.onChange(e); }}
                  />
        ...
        <Button type="submit" size="lg" disabled={isPending} className="w-full">
          {sending ? (
            ...
            <>Send reset code <IconArrowRight size={16} stroke={2} data-icon="inline-end" /></>
          )}
        </Button>
      </form>
    );
      ...
      <form onSubmit={otpForm.handleSubmit((d) => verifyCode(email, d.otp))} noValidate className="flex flex-col gap-5">
          ...
          <div className="mt-3">
              ...
              render={({ field, fieldState }) => (
                <Field data-invalid={fieldState.invalid}>
                  <InputOTP
                    ...
                    value={field.value}
                    onChange={(v) => { clearErrors(); field.onChange(v); }}
                    disabled={verifying}
                    containerClassName="w-full"
                  >
                    <InputOTPGroup className="w-full">
                      {[0,1,2,3,4,5].map((i) => (
                        <InputOTPSlot key={i} index={i} aria-invalid={fieldState.invalid} className="flex-1 h-9" />
                      ))}
                    </InputOTPGroup>
                  </InputOTP>
                  {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
            ...
            <div className="mt-3 flex items-center justify-center gap-1.5">
              ...
              {countdown.isActive ? (
                ...
                <button type="button" onClick={() => { resend(email); otpForm.reset(); clearErrors(); }} disabled={resending}
        ...
        <Button type="submit" size="lg" disabled={isPending} className="w-full">
          {verifying ? (
            ...
            <><IconCheck size={16} stroke={2} data-icon="inline-start" />Verify code</>
          )}
        </Button>
      </form>
    );
  ...
  return (
    <form onSubmit={passwordForm.handleSubmit((d) => resetPwd(resetToken, d.password))} noValidate className="flex flex-col gap-5">
      <div className="grid grid-cols-3 items-center">
        <span />
        <StepDots total={STEPS.length} current={2} className="justify-self-center" />
      </div>
      <div key={stepIndex} className={cn("flex flex-col gap-1.5", animClass)} style={{ animationDuration: "200ms" }}>
        <h2 className="text-base font-semibold text-foreground">{step.heading}</h2>
        <p className="text-sm text-muted-foreground">{step.description({ email })}</p>
      ...
      <Button type="submit" size="lg" disabled={isPending} className="w-full">
        {resetting ? (
          <><IconLoader2 size={16} stroke={2} className="animate-spin" data-icon="inline-start" />Saving password…</>
        ) : (
          <><IconCheck size={16} stroke={2} data-icon="inline-start" />Reset password</>
        )}
      </Button>
    </form>
  );
