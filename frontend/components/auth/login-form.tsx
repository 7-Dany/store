...
export function LoginForm({ initialIdentifier = "" }: LoginFormProps) {
  ...
  const { execute, isPending, error: apiError, clearError } = useLogin();
  ...
  // propagate server error onto the password field
  useEffect(() => {
    if (apiError) passwordForm.setError("password", { message: apiError });
  }, [apiError, passwordForm]);
