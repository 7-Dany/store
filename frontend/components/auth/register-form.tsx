...
const STEPS: { key: StepKey; heading: string; description: (ctx: { name: string; email: string }) => string }[] = [
...
export function RegisterForm() {
  ...
  const { execute: register, isPending: registering, error: registerError, clearError: clearRegisterError } = useRegister({
    onSuccess: () => advance(),
    onEmailTaken: () => { goTo(1); emailForm.setError("email", { message: "An account with that email already exists." }); },
  });

  const { execute: verify, isPending: verifying, error: verifyError, clearError: clearVerifyError } = useVerifyEmail();

  const { resend, isPending: resending, countdown } = useResendVerification();

  useEffect(() => { if (registerError) passwordForm.setError("password", { message: registerError }); }, [registerError, passwordForm]);
  useEffect(() => { if (verifyError)   otpForm.setError("otp",      { message: verifyError });   }, [verifyError, otpForm]);
  useEffect(() => { if (step.key === "otp") countdown.start(120); }, [stepIndex]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  function goTo(i: number) { clearErrors(); setDirection(i > stepIndex ? "forward" : "back"); setStepIndex(i); }

  function clearErrors() {
    clearRegisterError(); clearVerifyError();
    nameForm.clearErrors(); emailForm.clearErrors(); passwordForm.clearErrors(); otpForm.clearErrors();
  }
  ...
  function handleTextSubmit(data: Record<string, string>) {
    ...
    else { register(name, email, data.password); setPassword(data.password); }
  }
      ...
      <div key={stepIndex} className={cn("flex flex-col gap-1.5", animClass)} style={{ animationDuration: "200ms" }}>
        ...
        <FieldGroup className="mt-3 gap-0">
          <Controller
            ...
            render={({ field, fieldState }: { field: { onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; onBlur: () => void; value: string; ref: (el: HTMLInputElement | null) => void; name: string }; fieldState: { invalid: boolean; error?: { message?: string } } }) => (
              <Field data-invalid={fieldState.invalid}>
      ...
      <Button type="submit" size="lg" disabled={isPending} className="w-full">
        {isPending ? (
          <><IconLoader2 size={16} stroke={2} className="animate-spin" data-icon="inline-start" />
            {step.key === "password" ? "Creating account…" : "Loading…"}
          </>
        ) : (
