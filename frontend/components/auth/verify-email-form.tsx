...
export function VerifyEmailForm({ initialEmail }: Props) {
  const { execute: verify, isPending: verifying, error: verifyError, clearError } = useVerifyEmail();
  ...
  useEffect(() => { if (verifyError) form.setError("otp", { message: verifyError }); }, [verifyError, form]);

  // A code was already sent by useLogin before the redirect — start cooldown.
  useEffect(() => {
    countdown.start(120);
  ...
  return (
    <form onSubmit={form.handleSubmit((d) => verify(initialEmail, d.otp))} noValidate className="flex flex-col gap-5">
      <div className="flex flex-col gap-1.5">
        ...
        <p className="text-sm text-muted-foreground">We sent a 6-digit code to your email. Enter it below to verify your account.</p>
        {initialEmail && (
        ...
        <div className="mt-3">
            ...
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                  <InputOTP
                    ...
                    value={field.value}
                    onChange={(v) => { clearError(); field.onChange(v); }}
                    disabled={verifying}
          ...
          <div className="mt-3 flex items-center justify-center gap-1.5">
            ...
            {countdown.isActive ? (
              ...
              <button
                type="button"
                onClick={() => { resend(initialEmail); form.reset(); clearError(); }}
                disabled={resending}
