...
const STEPS: {
  ...
  },
  {
    ...
    description: ({ name }) =>
      `Nice to meet you${name ? `, ${name.split(" ")[0]}` : ""}! We'll send a verification code here.`,
  },
  {
    key: "password",
...
export function RegisterForm() {
  ...
  if (step.key === "otp") {
    return (
      <form
        onSubmit={otpForm.handleSubmit((d) => verify(email, d.otp))}
        noValidate
      ...
      >
        ...
        <Button type="submit" size="lg" disabled={isPending} className="w-full">
          {verifying ? (
            ...
            <>
              <IconCheck size={16} stroke={2} data-icon="inline-start" />
              Verify email
            </>
          )}
        </Button>
  ...
  function handleTextSubmit(data: Record<string, string>) {
    if (step.key === "name") {
      ...
      register(name, email, data.password);
    }
  }

  const fieldName = step.key as "name" | "email" | "password";
  const currentField = step.key === "password" ? passwordForm.watch("password") ?? "" : "";

  return (
    <form
      onSubmit={activeForm.handleSubmit(handleTextSubmit as never)}
      noValidate
      className="flex flex-col gap-5"
    >
      <div className="grid grid-cols-3 items-center">
          ...
          <Controller
            ...
            render={({
              ...
              <Field data-invalid={fieldState.invalid}>
                ...
                {step.key === "password" && (
                  <PasswordStrength value={currentField} />
                )}
                {fieldState.invalid && (
                  <FieldError errors={[fieldState.error]} />
                )}
