...
export function DangerZone() {
  ...
  return (
    <>
      <div className="flex items-start justify-between gap-6 rounded-xl border border-destructive/25 bg-destructive/3 dark:bg-destructive/5 p-4">
        <div className="flex flex-col gap-0.5">
          <p className="text-sm font-medium text-foreground">Delete account</p>
          <p className="text-xs text-muted-foreground leading-relaxed">
            Schedules your account for permanent deletion after a 30-day grace
            period. All data, sessions, and connected accounts will be removed.
          </p>
        </div>
        <Button
          variant="destructive"
...
function DeleteAccountDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent
        showCloseButton={state.step !== "deleting" && state.step !== "done"}
      >
        {(state.step === "loading_method" ||
          state.step === "confirm_otp_init") && (
        ...
        {state.step === "deleting" && (
          <SpinnerStep label="Scheduling account deletion…" />
        )}
  ...
  return (
    <>
      <DialogHeader>
        ...
        <DialogDescription>
          This schedules your account for permanent deletion after a 30-day
          grace period. Enter your password to confirm.
        </DialogDescription>
      </DialogHeader>
